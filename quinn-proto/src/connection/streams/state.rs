...
mod tests {
    ...
    #[test]
    fn lazy_allocation_does_not_reopen_closed_streams() {
        let mut client = make(Side::Client);
        let id0 = StreamId::new(Side::Server, Dir::Uni, 0);
        let id5 = StreamId::new(Side::Server, Dir::Uni, 5);
        assert_eq!(
            client.received(
                frame::Stream {
                    id: id0,
        ...
        assert!(chunks.next(0).unwrap().is_none());
        let _ = chunks.finalize();
        assert!(!client.recv.contains_key(&id0));
        assert_eq!(client.initialized_remote_count[Dir::Uni as usize], 1);
        ...
        assert_eq!(client.initialized_remote_count[Dir::Uni as usize], 6);
        assert_eq!(client.recv.len(), 5);
        assert!(!client.recv.contains_key(&id0));
        assert!(client.recv.contains_key(&id5));
    }
