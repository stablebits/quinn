...
impl TestEndpoint {
    ...
    pub(super) fn start_split_accept(
        &mut self,
        incoming: Incoming,
        now: Instant,
    ) -> crate::endpoint::Accepting {
        let mut buf = Vec::new();
        self.endpoint
            .start_accept_inner(incoming, now, &mut buf, None)
            .unwrap()
    }

    pub(super) fn finish_split_accept(
        &mut self,
        accepting: crate::endpoint::Accepting,
    ) -> ConnectionHandle {
        let Ok(accepted) = accepting.finish_without_endpoint() else {
            panic!("split accept unexpectedly failed")
        };
        let (ch, conn) = self.endpoint.finish_accept_inner(accepted);
        self.connections.insert(ch, conn);
        ch
    }
    ...
    pub(super) fn finish_split_accept_error(
        &mut self,
        accepting: crate::endpoint::Accepting,
    ) -> ConnectionError {
        let mut buf = Vec::new();
        ...
        self.endpoint
            .finish_accept_error_inner(error, &mut buf)
            .cause
    }
